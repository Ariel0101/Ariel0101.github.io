        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            if (textureColor.z > textureColor.x && textureColor.z > textureColor.y){
                position+=normal*(1.0+0.3*sin(uv.x*26.0*PI+time*28.0))*0.03;
                position+=normal*(1.0+0.3*sin(uv.y*26.0*PI+time*28.0))*0.03;
            } else {
                position+=normal*0.1;
            }
